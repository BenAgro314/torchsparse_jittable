...
def sphash(
    ...
    if offsets is None:
        if coords.device.type == "cuda":
            return torch.ops.torchsparse_ops.hash_cuda(coords)
        elif coords.device.type == "cpu":
            return torch.ops.torchsparse_ops.hash_cpu(coords)
        else:
            device = coords.device
            return torch.ops.torchsparse_ops.hash_cpu(coords.cpu()).to(device)
    else:
        ...
        if coords.device.type == "cuda":
            return torch.ops.torchsparse_ops.kernel_hash_cuda(coords, offsets)
        elif coords.device.type == "cpu":
            return torch.ops.torchsparse_ops.kernel_hash_cpu(coords, offsets)
        else:
            device = coords.device
            return torch.ops.torchsparse_ops.kernel_hash_cpu(coords.cpu(), offsets.cpu()).to(
                device
            )
