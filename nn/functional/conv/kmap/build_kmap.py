...
from torchsparse.utils import make_nlist, make_tensor, make_divisible
...
def build_kernel_map(
    ...
    hashmap_keys: Optional[torch.Tensor] = None,
    hashmap_vals: Optional[torch.Tensor] = None,
    spatial_range: Optional[Union[int, List[int]]] = None,
    mode: str = "hashmap",
    dataflow: int = Dataflow.ImplicitGEMM.value,
    ...
    if spatial_range is not None and torch.jit.isinstance(spatial_range, List[int]):
        assert torch.jit.isinstance(spatial_range, List[int])
        new_spatial_range = [spatial_range[0]]
        for i in range(3):
            new_spatial_range.append((
    ...
    if mode == "hashmap_on_the_fly":
        ...
        if dataflow == Dataflow.ImplicitGEMM.value:
        ...
        elif dataflow == Dataflow.GatherScatter.value:
        ...
        elif dataflow == Dataflow.FetchOnDemand.value:
    ...
    if dataflow == Dataflow.ImplicitGEMM.value:
        if training:
            ...
            out_in_map_bwd = F.convert_transposed_out_in_map(
                out_in_map, make_divisible(sizes[0], cta_M)
            )
            bitmask_bwd = torch.ops.torchsparse_ops.derive_bitmask_from_out_in_map(
                out_in_map_bwd, split_mask_num_bwd, sizes[0]
            )
            sorted_mask_bwd, reorder_loc_bwd = torch.sort(bitmask_bwd, descending=True)
            reorder_loc_bwd = reorder_loc_bwd.to(torch.int32)
            reorder_out_in_map_bwd = torch.ops.torchsparse_ops.reorder_out_in_map_cuda(
                out_in_map_bwd, reorder_loc_bwd
            )
            reduced_sorted_mask_bwd_wgrad = torch.ops.torchsparse_ops.reduce_bitmask_cuda(
                sorted_mask_bwd, cta_M_wgrad
            )
            reduced_sorted_mask_bwd_dgrad = torch.ops.torchsparse_ops.reduce_bitmask_cuda(
...
def transpose_kernel_map(
    ...
    assert torch.jit.isinstance(out_in_map_bwd, torch.Tensor)
    sizes = kmap["sizes"]
    assert torch.jit.isinstance(sizes, Tuple[int, int])
    out_in_map = F.convert_transposed_out_in_map(
        out_in_map_bwd, make_divisible(sizes[0], cta_M)
    )
    ...
    if ifsort:
        if training:
            ...
            assert torch.jit.isinstance(sorted_mask_bwd, torch.Tensor)
            reduced_sorted_mask_bwd_wgrad = torch.ops.torchsparse_ops.reduce_bitmask_cuda(
                sorted_mask_bwd, cta_M_wgrad
            )
            reduced_sorted_mask_bwd_dgrad = torch.ops.torchsparse_ops.reduce_bitmask_cuda(
        ...
        bitmask = torch.ops.torchsparse_ops.derive_bitmask_from_out_in_map(
            out_in_map, split_mask_num, sizes[0]
        )
        ...
        if training:
            bitmask_bwd = torch.ops.torchsparse_ops.derive_bitmask_from_out_in_map(
                out_in_map_bwd, split_mask_num_bwd, sizes[1]
            )
            sorted_mask_bwd, reorder_loc_bwd = torch.sort(bitmask_bwd, descending=True)
            reorder_loc_bwd = reorder_loc_bwd.to(torch.int32)
            reorder_out_in_map_bwd = torch.ops.torchsparse_ops.reorder_out_in_map_cuda(
