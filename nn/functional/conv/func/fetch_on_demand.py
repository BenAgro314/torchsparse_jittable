...
def forward_fetch_on_demand(
    ...
    if input.device.type == "cuda":
        ...
        if config.FOD_fusion == True:
            output = torch.ops.torchsparse_ops.conv_forward_fetch_on_demand_cuda(
                input,
                weight,
                nbmaps,
                mapsize,
            ...
            output = (
                torch.ops.torchsparse_ops.conv_forward_fetch_on_demand_no_fusion_cuda(
                    input,
                    weight,
                    nbmaps,
                    nbsizes.cpu(),
...
class FetchOnDemandConvolutionFuntion(Function):
    ...
    def forward(
        ...
        nbmaps = kmap["nbmaps"]
        nbsizes = kmap["nbsizes"]
        output = forward_fetch_on_demand(input, weight, kmap, config, transposed)
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == "cuda":
            torch.ops.torchsparse_ops.conv_backward_gather_scatter_cuda(
                input,
                grad_input,
            ...
            torch.ops.torchsparse_ops.conv_backward_gather_scatter_cpu(
                input,
                grad_input,
                grad_output.contiguous(),
                weight,
