...
def forward_gather_scatter(
    ...
    if input.device.type == "cuda":
        ...
        output = torch.ops.torchsparse_ops.conv_forward_gather_scatter_cuda(
            input,
            weight,
            nbmaps,
            nbsizes.cpu(),
        ...
        torch.ops.torchsparse_ops.conv_forward_gather_scatter_cpu(
...
class GatherScatterConvolutionFuntion(Function):  # TorchSparse_v2
    ...
    def forward(
        ...
        nbmaps = kmap["nbmaps"]
        nbsizes = kmap["nbsizes"].cpu()
        output = forward_gather_scatter(input, weight, kmap, config, transposed)
