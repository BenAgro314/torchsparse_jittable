...
class DevoxelizeFunction(Function):
    ...
    def forward(
        ...
        if feats.device.type == "cuda":
            output = torch.ops.torchsparse_ops.devoxelize_forward_cuda(feats, coords, weights)
        elif feats.device.type == "cpu":
            output = torch.ops.torchsparse_ops.devoxelize_forward_cpu(feats, coords, weights)
        else:
            device = feats.device
            output = torch.ops.torchsparse_ops.devoxelize_forward_cpu(
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == "cuda":
            grad_feats = torch.ops.torchsparse_ops.devoxelize_backward_cuda(
                grad_output, coords, weights, input_size
            )
        elif grad_output.device.type == "cpu":
            grad_feats = torch.ops.torchsparse_ops.devoxelize_backward_cpu(
                grad_output, coords, weights, input_size
            )
        else:
