...
class Conv3d(nn.Module):
    def __init__(
        ...
        in_channels: int,
        out_channels: int,
        kernel_size: Union[int, List[int], List[int]] = 3,
        stride: Union[int, List[int], List[int]] = 1,
        ...
        self.kernel_size = make_nlist(kernel_size, ndim=3)
        self.stride = make_nlist(stride, ndim=3)
        self.dilation = dilation
        _padding = make_nlist(padding, 3)
        self.padding = []
        for i in range(3):
            if self.kernel_size[i] % 2 == 1 and self.stride[i] == 1:
                self.padding.append((self.kernel_size[i] - 1) // 2,)
            else:
                self.padding.append(_padding[i],)
        ...
        self.allow_tf32 = torchsparse.backends.allow_tf32
        self.allow_fp16 = torchsparse.backends.allow_fp16
